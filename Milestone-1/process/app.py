...
# New (working) setting
_GEMINI_MODEL = os.getenv('GEMINI_MODEL', 'gemini-1.5-flash')  # Changed to flash for faster responses
...
def chatbot_page():
    ...
    if 'chat_session_id' not in session:
        session['chat_session_id'] = f"session_{int(datetime.now().timestamp())}"

    # Fetch chat history (using the new GeminiChat model)
    history = GeminiChat.query.filter_by(user_id=current_user.id).order_by(GeminiChat.created_at.asc()).limit(50).all()

    return render_template('chatbot.html', history=history, session_id=session.get('chat_session_id'))
